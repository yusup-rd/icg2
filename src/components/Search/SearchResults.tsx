...
const SearchResults = ({ query }: { query: string }) => {
  ...
  return (
    <div className="z-20 mt-4 w-full rounded-lg bg-card p-1 ring-2 ring-border md:absolute md:p-0">
            ...
            {mockResultGames.map((game) => (
              <div key={game.id} className="w-full">
                <Image
                  ...
                  height={200}
                  priority={game.id === 0}
                  className="mb-3 rounded object-contain duration-300 ease-in-out hover:-translate-y-2 hover:shadow-md cursor-pointer"
                />
              </div>
            ))}
          </div>
